import { Resend } from 'resend'                                                                                                      
   ...
   const resend = new Resend(process.env.RESEND_API_KE Y as string)                                                                     
   ...
   export async function sendChangeAlert(to: string, name: string, summary: string, url: string) {                                      
     await resend.emails.send({                                                                                                         
       ...
       html: `<div style="font-family:sans-serif;pa dding:24px">                                                                        
   ...
   export async function sendWeeklyDigest(to: string, changes: { competitorName: string; summary: string; url: string }[]) {            
     const html = changes                                                                                                               
       .map(c => `<div style="margin-bottom:16px;paddin g:16px;border:1px solid #e5e7eb;border-radius:8px">                             
