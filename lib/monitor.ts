...
export async function analyzeBaseline(content: string, name: string): Promise<string> {
  ...
  return r.choices[0].message.content || 'Baseline established.'
}
...
export async function runMonitorForCompetitor(competitorId: string) {
  ...
  if (!last) {
    // First scan — generate baseline intelligence briefing so something shows immediately
    hasChanged = true
    const summary = await analyzeBaseline(currentContent, competitor.name)
    changesDetected = { summary, detected_at: new Date().toISOString(), type: 'baseline' }
  } else if (last.content_hash !== currentHash) {
    // Content changed — analyze what changed
    hasChanged = true
    const summary = await analyzeChanges(last.content || '', currentContent, competitor.name)
    changesDetected = { summary, detected_at: new Date().toISOString(), type: 'change' }
  }
